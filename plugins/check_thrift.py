    ...
    def __init__(self, node, alert_conf):
        ...
        if not self.is_alive:
            self.riemann_client.send(host='%s-%s' % (self.host, self.port),
                                     service='Sina_dsp_thrift_conn_fail',
                                     ip_port='%s-%s' % (self.host, self.port),
                                     description='thrift conn fail',
                                     alert_level=self.check_items['Sina_dsp_thrift_conn_fail'][0][1],
                                     threshold=self.check_items['Sina_dsp_thrift_conn_fail'][0][0],
                                     sms=self.check_items['Sina_dsp_thrift_conn_fail'][0][2],
                                     mail=self.check_items['Sina_dsp_thrift_conn_fail'][0][3],
                                     watchid=self.check_items['Sina_dsp_thrift_conn_fail'][0][4])
